...
const ImageUploader = ({onUpload, images, onRemove}) => {
    ...
    const handleChange = async (e) => {
        ...
        setStat('pending');
        await axios.post(`${SERVER}/upload/image`,fd)
        .then(res => {
    ...
    return (
        <div className={cx('image-upload', images.length !== 0 ? 'justify-start' : 'justify-center')}>
            {
                images.map(e => {
                    return(
            ...
            {
            (images.length < 5 && stat !== 'pending') &&
            <label className={cx(images.length !== 0 ? 'upload-button' : 'primary-upload-button')}>
